...
Lagrange's theorem relates the size of a subgroup to the size of the ambient group via the notion of index, and provides a basic constraint on possible subgroup structures in finite groups.
...
== Lean
Lean is an interactive proof assistant based on _dependent type theory_, in which propositions are represented as types, and proofs correspondent to terms inhabiting these types. This view @love allows treating mathematical statements and proofs as structured objects that can be manipulated and checked by the system. To keep logical coherence, types in Lean form an infinite hierarchy of universes, identified by levels. Propositions live in `Prop` equiv `Sort 0` while others types live in `Type u` $equiv$ `Sort (u + 1)`.

In Lean, a mathematical theorem is introduced by keywords such as `theorem` or `lemma`, followed by a name, a list of parameters, and type annotations of this theorem. A proof is then provided by constructing a term of this type. It is correct, if it _type checks_. 
...
theorem lagrange {G : Type*} [Group G] [Fintype G] (H : Subgroup G) :
    ...
    Fintype.card (a • (H : Set G) : Set G) := by sorry
```
In the example above, `lagrange` is the theorem's name. The expression between the name and colon specify the parameters and assumptions of the theorem. The expression between the colon and colon equals is the type of the theorem. The colon equals introduces the proof, and the code following it constitutes the corresponding proof term. When the keyword `by` is used, the proof is written in tactic mode. Tactics are tools that facilitate the construction of proof terms. When a tactic is applied, we can observe in the `Info-View` of our editor that the proof state has been modified.
...
_Type classes_ serve multiple purposes in the language manual@lean4_ref. In this thesis, they are primarily used to represent algebraic structures together with the axioms they satisfy. When a type class parameter is required, Lean automatically searches for a suitable instance registered in the environment. This process can also be invoked explicitly using the command `inferInstance`, which instructs Lean to construct the appropriate instance if one is available.

Another important mechanism named _coercion_, done by Lean automatically when a term is used in a context expecting a compatible type. For instance, coercions are frequently used to pass from `Nat` to `Int`, or from algebraic structures to their underlying carrier types. But sometimes we also need to do coercions manually. In the example above, it appears in `(a • (H : Set G) : Set G)`. Here, the subgroup `H : Subgroup G` is explicitly coerced to its underlying set `H : Set G`.


== Mathlib 
Mathlib is currently one of the biggest formal math library, containing extensive subjects. However, rather than being merely a large collection of formalized theorems, mathlib is shaped by a design philosophy centered on abstraction, generalization and systematic reuse through typeclass inference. The library favors small composable lemmas that act as bridges, enabling the construction of more complex results in a modular way@baanen2025. The design philosophy becomes particularly visible in its algebraic hierarchy, where algebraic structures are not defined independently, but through several layers by using type classes. A representative example is the definition of a group in mathlib:
```lean
...
Due to the scale and complexity of the library, efficient navigation becomes an important skill in this project. Locating exisiting lemmas is crucial tp avoid duplication and to reuse established results. The mathlib documentation@doc_mathlib usually serves as the primary entry point. It is organized into thematic cateogries such as `GroupTheory`, `FieldTheory`, each containing several more specialied subtopics. For instance, `Coset`, `Subgroup`, `QuotientGroup` are subtopics under `GroupTheory`. Furthermore, under each subtopic, definitions are collected into section `Defs`, basic theories are placed into `Basic`, and the core results are into other labels. Using the mathlib documentaion to locate a lemma usually requires knowing the name of the lemma. While during the formalization process, the name of a lemma might not always be obvious. Tools such as _LeanSearch_ @lean_search allows us to qury lemmas without fully knowing their names, while ineratcive tactic like `apply?` can give hints like a partial proof. In practice, most commonly needed results already exist within the library; thus, working within mathlib often involves identifying and adapting existing components rather than constructing proofs entirely from scratch.
