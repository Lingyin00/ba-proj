...
In this section, I formalize two theorems describing the behavior of Hall subgroups with respect to normal subgroups - namely under intersection and under forming products with a normal subgroup and passing to the corresponding quotient group. Recall the definition 2.1.1, a subgroup is a Hall subgroup if its index and cardinality is coprime.
...
Since $N$ is a normal subgroup of $G$, the product of $H N$ forms a subgroup of $G$. By a standard counting argument (see Proposition.13 in Sec.3.2@Dummit) gives: $ |H N| = |H| dot |N| \/|H inter.sq N|. $ Rearranging the equation we get: 
...
As in the proof of the theorem above, we use
...
- if H is a normal subgroup of $G$, the cardinality of quotient group $|G : H| = |G \/ H|$.
- if G is a finite group, the ratio of cardinality (according to Lagrange theorem): $|G:H| = |G| \/ |H|$
...
```
The notation `G ⧸ N`, introduced via the `HasQuotient` type class, provides the quotient as a type. The assumption `[N.Normal]` is required to endow this type with a group structure via instance inference.
